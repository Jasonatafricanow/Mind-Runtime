...
from mind_runtime.contracts.host import (
    HostCommitRequest, HostProviderProseRequest, HostStatus, HostTurnRequest,
)
...
from mind_runtime.expression.guards import (
    DeterministicExpressionGuardChain, ExpressionGuardConfig,
)
...
def _stack(tmp_path, *, resources=("send_message",), max_render_chars=5000):
    ...
    binding = RuntimeBinding(
        persona_id=revision.persona_id, agent_id="fixture-persona",
    ...
    config = DecisionContextConfig(
        allowed_situation_facts=(), affect_rules=(),
        persona_style_constraints=(("format", "bullets"),),
        allowed_history_kinds=(), max_history_items=2,
        max_prior_expression_chars=100, max_item_chars=200,
        max_items=20, max_render_chars=max_render_chars, mode="SURFACE_V1",
    )
    rule = IntentRule(
        rule_id="contact", kind="reach_out", base_strength=0.5,
        dimension_weights=(), event_kind=None, event_bonus=0,
        minimum_strength=0.1, due_at_attribute=None, expires_after=None,
    ...
    policy = ActionPolicyConfig(
        rules=(IntentPolicyRule("reach_out", "send_message", False, False,
                                None, None, None),),
        proactive_cooldown=timedelta(0),
    )
    orchestrator, _ = build_runtime_stack(
        clock=FakeClock(datetime(2026, 9, 23, tzinfo=UTC)),
        facts_db=tmp_path / "facts.sqlite", state_db=tmp_path / "states.sqlite",
        origin_runtime_id="fixture-runtime", user_id="fixture-user",
        intent_rules=(rule,), intent_db=tmp_path / "intent.sqlite",
        action_policy_config=policy, policy_resources=resources,
        decision_context_config=config, persona_publication=publication,
        surface_binding_registry=registry, surface_binding_id="fixture-binding",
        surface_binding_environment=RuntimeEnvironment.LAB,
        delivery_db=tmp_path / "delivery.sqlite",
        expression_guard=DeterministicExpressionGuardChain(ExpressionGuardConfig(
            prefix_length=8, transport_markers=(), banned_openings=(), temporal_rules=(),
        )),
    )
    return orchestrator
...
def test_real_turn_handoff_commits_before_host_provider_and_survives_restart(tmp_path):
    ...
    assert orchestrator.surface_projection_port is orchestrator.cognitive_tick_components["ticker"].surface_projection_port
    host = MindRuntimeHostAdapter(orchestrator=orchestrator)
    result = host.begin_turn(HostTurnRequest(
        interaction_id="turn-1", runtime_id="fixture-runtime",
    ...
    conflicting = replace(request, surface_handoff=replace(
        request.surface_handoff, qualitative_guidance=altered_guidance,
    ))
    ...
    guard = host.guard_provider_prose(HostProviderProseRequest(
        turn_id=result.turn_id, interaction_id=result.interaction_id,
        prose="Hello from the provider.",
    ))
    assert guard.status is HostStatus.OK, guard.reason_codes
    committed = host.commit_turn(HostCommitRequest(
        turn_id=result.turn_id, interaction_id=result.interaction_id,
    ))
    ...
    replay = recover_admitted_surface_handoff(
        operational, request.request_id, origin_runtime_id="fixture-runtime",
        scope=request.scope,
    )
    ...
    with pytest.raises(ValueError, match="SURFACE_HANDOFF_BINDING_MISMATCH"):
        recover_admitted_surface_handoff(
            operational, request.request_id, origin_runtime_id="other-runtime",
            scope=request.scope,
        )
    operational.close()
...
def test_generic_c7_carrier_cannot_send_surface_provider_envelope(tmp_path):
    ...
    orchestrator = _stack(tmp_path)
    host = MindRuntimeHostAdapter(orchestrator=orchestrator)
    host.begin_turn(HostTurnRequest(
        interaction_id="turn-1", runtime_id="fixture-runtime",
        scope=Scope(domain=ScopeDomain.USER, user_id="fixture-user"),
        occurred_at=datetime(2026, 9, 23, tzinfo=UTC),
        user_message="hello", channel="chat",
...
def test_surface_handoff_budget_one_character_short_withholds_request(tmp_path):
    ...
    normal_host.begin_turn(HostTurnRequest(
        interaction_id="turn-1", runtime_id="fixture-runtime",
        scope=Scope(domain=ScopeDomain.USER, user_id="fixture-user"),
        occurred_at=datetime(2026, 9, 23, tzinfo=UTC),
        user_message="hello", channel="chat",
    ))
    essential = [item for item in normal.decision_context.expression_context
                 if item.kind in (ExpressionContextKind.ACTION,
    ...
    result = short_host.begin_turn(HostTurnRequest(
        interaction_id="turn-1", runtime_id="fixture-runtime",
        scope=Scope(domain=ScopeDomain.USER, user_id="fixture-user"),
        occurred_at=datetime(2026, 9, 23, tzinfo=UTC),
        user_message="hello", channel="chat",
...
def test_c7_commit_failure_withholds_provider_handoff(tmp_path, monkeypatch):
    ...
    host = MindRuntimeHostAdapter(orchestrator=orchestrator)
    provider_calls = []
    monkeypatch.setattr(orchestrator.expression_coordinator, "express",
...
def test_surface_handoff_cannot_commit_without_guard(tmp_path):
    ...
    receipt = host.commit_turn(HostCommitRequest(
        turn_id=result.turn_id, interaction_id=result.interaction_id,
    ))
...
def test_failed_later_guard_clears_prior_acceptance(tmp_path, monkeypatch):
    ...
    assert host.commit_turn(HostCommitRequest(
        turn_id=result.turn_id, interaction_id=result.interaction_id,
    )).status is HostStatus.FAILED
