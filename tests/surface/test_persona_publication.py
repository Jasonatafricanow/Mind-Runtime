"""ADR-0032: immutable config revision and cross-process conflict proofs."""
...
def _process(root: Path, action: str, source: str, version: int = 2, digest: str = ""):
    ...
    program = (
        "from mind_runtime.persona_publication import "
        "PersonaConfigPublicationRepository,PersonaRevisionRef;"
...
def test_missing_artifact_fails_without_mutable_alias_fallback(tmp_path: Path) -> None:
    ...
    ref = PersonaRevisionRef(
        loaded.persona_id, loaded.profile_version, loaded.effective_content_digest
    )
...
def test_restart_binding_resolves_exact_published_revision(tmp_path: Path) -> None:
    ...
    exact = restored.resolve_persona_revision("fixture-binding", environment=RuntimeEnvironment.LAB)
    assert exact == ref
