...
def test_zero_weight_causal_declarations_still_reject_overlap(
    ...
    with pytest.raises(ValueError, match="ROOT_OVERLAP"):
        intent_surface_validator(
            {
                "kind": "reach_out",
                "direct_dynamics_weights": direct,
                "surface_control_weights": controls,
                "event_bonus": 0.0,
            }
        )
...
def _sync(
    scope: Scope, object_id: str, version: int = 1, runtime_id: str = "fixture-runtime"
) -> SyncFields:
    return SyncFields(
        scope=scope,
        origin_runtime_id=runtime_id,
...
def test_surface_aware_rule_scores_tendency_and_emits_trace():
    ...
    semantic = {
        key: value
        for key, value in wrong_recipe.items()
