...
def test_candidate_recipe_and_persona_digests():
    ...
    assert (
        digest("persona", normalized_persona_content(x_a["persona"]["content"])) == PERSONA_A_DIGEST
    )
    assert digest("dependencies", dependency_payload(x_a)) == DEP_A_DIGEST

    x_b = sample_candidate("persona-fixture-b")
    assert (
        digest("persona", normalized_persona_content(x_b["persona"]["content"])) == PERSONA_B_DIGEST
    )
