...
def recompute_committed_surface(
    *,
    ...
    if not isinstance(state_backend, SqliteStateBackend) or not isinstance(
        commit_markers, SqliteCommitMarkerStore
    ):
    ...
    *,
    ...
    states = [
        {
            ...
            if persona.for_dimension(s.dimension) is not None
            else None,
            "value_type": "scalar",
    ...
    content = {
        ...
        "behavioral_disposition": {
            "attachment_approach": float(persona.behavioral_disposition["attachment_approach"]),
            "confrontation_readiness": float(
                persona.behavioral_disposition["confrontation_readiness"]
            ),
            "expressive_restraint": float(persona.behavioral_disposition["expressive_restraint"]),
