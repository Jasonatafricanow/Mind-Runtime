"""Runtime-independent Memory core over one canonical MR database."""
...
class MemoryCore:
    """Reusable canonical Memory + local product projection ownership surface."""
    ...
    def select(
        ...
        *,
        ...
        if (
            ...
            or not 1 <= len(memory_ids) <= max_items
        ):
        ...
        if len(set(memory_ids)) != len(memory_ids):
            raise MemoryCoreSelectionError(
                "duplicate Memory IDs are not permitted"
            )
        ...
        if scope is not None and any(
            memory.scope != scope for memory in resolved
        ):
            raise MemoryCoreSelectionError(
