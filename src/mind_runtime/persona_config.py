    ...
    def register(
        ...
        if existing is not None and existing != effective_content_digest:
            raise PersonaProfileError(
                f"persona content conflict for persona_id={persona_id!r}, revision={profile_version}: "
    ...
    *,
    ...
    if "schema_version" not in raw:
        ...
        _require(
            ...
            f"{file}: unsupported schema_version {raw_schema!r} (supported: {SUPPORTED_SCHEMA_VERSIONS})",
        )
