...
never used to fabricate an effective interval. Multiple supporting Evidence
timestamps remain separate support chronology.

A Memory may be associated with more than one Reality Observation. The binding
...
Unknown temporal fields remain unknown. Later Evidence can create a new Memory
and new cognition, but cannot mutate the earlier Memory view at its historical
cutoff.
...
## Current bounded ambiguities

Reality Observation observed_at intentionally equals the original
Evidence.received_at under the frozen Reality contract. It is causal knowledge
chronology, not an extractor-processing timestamp. If a future audit requires
the wall-clock instant at which a semantic extractor actually ran, that must be
a new explicit processing field; observed_at must not be repurposed.

The default text-v1 Memory admission currently emits one supporting Evidence per
Memory. The temporal contract nevertheless preserves multiple provenance refs
without converting their timestamp span into proposition-valid time.
