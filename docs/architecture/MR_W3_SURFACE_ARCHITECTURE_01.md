...
Date: 2026-09-23. Status: architecture decision for review; no production implementation. Verdict: **ARCHITECTURE_READY_FOR_W3_IMPLEMENTATION**, subject to accepting the companion ADR and replacing the affected RED contract before code work under AGENTS.md.
...
The W2 appraisal/Dynamics path may continue using legacy profiles. SURFACE_V1 cannot start on one. The same profile snapshot used for Dynamics is bound into Surface through an explicit runtime-owned provenance envelope, not inferred from scope or a later mutable config read. Future longitudinal personality evolution can publish a new profile revision through a separate authority and migration; W3 defines only this consumption seam.
...
| REPLACE | G15/G18/G19 exact numeric values, recipe AST/binary64 digests, sample clamp trace and exact sample tone bands | Keep the old values only under an explicitly named reference-fixture suite. New production Golden vectors must be generated from an admitted versioned production recipe and renderer mapping, with independent expected oracles. The 98 RED count is not a target metric. |
...
The Golden contract is currently untracked on a different base and has no W2 Intent/Host end-to-end coverage. Its normative claims are the semantic invariants above, not its sample coefficients or exact wire DTO. Existing fixture-integrity GREEN does not certify Surface production behavior.

## 15. Minimal implementation slices and gates

| Slice | Narrow authority changed | Independent acceptance |
|---|---|---|
| W3-A authority/config | PersonaProfile and loader; schema/content revision split; immutable four-trait block; recipe/mapping/config version admission | Same ID/revision content conflict, legacy separation, missing/invalid block failure; no Surface behavior or W2 source change. |
| W3-B pure Surface | New single Surface contract/port and composition; declared roots and deterministic recipe evaluation | Counterfactuals, complete five-key vector, isolation, lineage, restart reconstruction, abort, no I/O/model/clock, no Dynamics mutation; production recipe explicitly pinned. |
...
| W3-E certification | Tests and independent review only | W2 invariant regression, complete current suite, production-shaped turn and tick, restart/abort, real provider-visible Surface; exact HEAD/tree and environment evidence. No role-card benchmark. |

Each slice starts with an accepted ADR/contract and a reviewed RED proof for its own authority change under AGENTS.md. No slice silently alters W2 AcceptedAppraisal, AppraisalProjector, Dynamics sensitivity, receipt, or FACT boundary. Mainline integration, merge and push are outside this architecture task.
...
**ARCHITECTURE_READY_FOR_W3_IMPLEMENTATION.** The production topology, ownership, typed roots, fail-closed scoring and exposure seams are specified without changing W2. This is an architecture verdict, not evidence that Surface exists or permission to merge/push. Repository change control still requires acceptance of the companion ADR, revised RED contracts, and an admitted production recipe before activating SURFACE_V1.
